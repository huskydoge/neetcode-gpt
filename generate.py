...
class Solution:
    def generate(self, model, new_chars: int, context: TensorType[int], context_length: int, int_to_char: dict) -> str:
        ...
        for i in range(new_chars):
            ...
            print(outputs.shape)
            next_probs = torch.softmax(outputs, dim = -1)[:, -1, :]
            print(next_probs.shape)
            next_token = torch.multinomial(next_probs, 1, generator = generator)
            contexts = torch.cat((context, next_token), dim = 1)
            res.append(int_to_char[next_token.item()])
            # The line where y
            # ou call torch.multinomial(). Pass in the generator as well.
