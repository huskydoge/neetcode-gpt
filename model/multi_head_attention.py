...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.attention_list = nn.ModuleList([self.SingleHeadAttention(embedding_dim, head_size) for _ in range(num_heads)])
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        for attn in self.attention_list:
            logits = attn(embedded)
            print(logits.shape)
            logits_list.append(logits)

        concated_logits = torch.cat(logits_list, dim = 2)

        output = self.o_proj(concated_logits)
